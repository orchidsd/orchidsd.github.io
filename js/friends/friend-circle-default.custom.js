function initialize_fc_lite() {
    UserConfig = {
        ...
        lost_contact_num: UserConfig?.lost_contact_num ?? -1
    };
    ...
    function initializeTopSection() {
        ...
        const lostNum = getDataSource() === 'private' && UserConfig.lost_contact_num >= 0 ? UserConfig.lost_contact_num : globalStats.error_num;

        const statsCard = `<div class="stats-card-container"><div class="stats-card-left"><div class="stats-item"><div class="stats-value">${globalStats.friends_num}</div><div class="stats-label">订阅</div></div><div class="stats-item"><div class="stats-value">${globalStats.active_num}</div><div class="stats-label">活跃</div></div><div class="stats-item"><div class="stats-value">${globalStats.article_num}</div><div class="stats-label">文章</div></div><div class="stats-item"><div class="stats-value">${lostNum}</div><div class="stats-label">失联</div></div></div><div class="stats-card-right"><div class="stats-controls"><button id="sort-toggle-btn">${sortRule==='created'?'发布时间':'更新时间'}</button><button id="source-toggle-btn">${sourceRule==='private'?'私有订阅':'公共订阅'}</button></div><div class="stats-version"><span id="version-text">加载中</span><span id="version-info">正在与主机通讯中</span></div></div></div>`;
