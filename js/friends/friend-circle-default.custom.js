function initialize_fc_lite() {
    ...
    const fishPondHeader = document.createElement('div');
    fishPondHeader.className = 'fish-pond-header';
    fishPondHeader.innerHTML = '<span class="fish-pond-title">🐟 鱼塘</span>';
    root.appendChild(fishPondHeader);
    ...
    function processArticles(data, skipTop) {
    ...
    function initializeTopSection() {
        ...
        const statsCard = `<div class="stats-card-container"><div class="stats-card-left"><div class="stats-item"><div class="stats-value">${globalStats.friends_num}</div><div class="stats-label">订阅</div></div><div class="stats-item"><div class="stats-value">${globalStats.active_num}</div><div class="stats-label">活跃</div></div><div class="stats-item"><div class="stats-value">${globalStats.article_num}</div><div class="stats-label">文章</div></div><div class="stats-item"><div class="stats-value">${globalStats.error_num}</div><div class="stats-label">失联</div></div></div><div class="stats-card-right"><div class="stats-controls"><button id="sort-toggle-btn">${sortRule==='created'?'发布时间':'更新时间'}</button><button id="source-toggle-btn">${sourceRule==='private'?'私有订阅':'公共订阅'}</button></div><div class="stats-version"><span id="version-text">加载中</span><span id="version-info">正在与主机通讯中</span></div></div></div>`;
        ...
        document.getElementById('sort-toggle-btn').addEventListener('click', () => {
            const current = getSortRule();
            const next = current === 'created' ? 'updated' : 'created';
            setSortRule(next);
            const btn = document.getElementById('sort-toggle-btn');
            if (btn) btn.textContent = next === 'created' ? '发布时间' : '更新时间';
            start = 0;
            container.innerHTML = '';
            processArticles({ article_data: allArticles, statistical_data: globalStats }, true);
        });
        ...
        document.getElementById('source-toggle-btn').addEventListener('click', () => {
            ...
            if (btn) btn.textContent = next === 'private' ? '私有订阅' : '公共订阅';
            clearCache();
            start = 0;
            container.innerHTML = '';
            loadMoreArticles();
    ...
    async function checkVersionUpdate() {
        ...
        try {
            ...
            if (!localVersionResponse.ok) {
                versionText.textContent = '公共订阅';
                versionText.className = 'version-latest';
                versionText.title = '公共订阅无版本信息';
                versionInfo.textContent = '公共订阅（示例池）';
                return;
            }
            ...
            const lp = parseVersion(normalizedLocal);
            const rp = parseVersion(normalizedLatest);
            ...
            if (cmp === 0) {
                ...
                versionText.title = '有新版本可更新';
                versionInfo.textContent = `新版本可用:${latestVersion.trim()}`;
            } else {
                // 本地比官方新 - 绿色，正常显示
                versionText.className = 'version-latest';
                versionText.title = '本地版本已是最新';
                versionInfo.textContent = '已是最新';
            }
        } catch (error) {
            // 获取失败 - 红色
