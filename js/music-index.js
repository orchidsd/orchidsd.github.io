(function () {
  ...
  let anzhiyuPlayerState = {
    ...
    lyricsContainerElement: null,
    lyricsInnerElement: null,
    lyricsLines: [],
    lyricsTimes: [],
  ...
  function parseLrc(lrcText) {
    if (typeof window.__parseLrc === "function") return window.__parseLrc(lrcText);
    const lines = [];
    const times = [];
    if (!lrcText) return { lines, times };
  ...
  async function loadTrackLyrics() {
    ...
    globalState.trackLyrics = parsed.lines;
    globalState.lyricsTimes = parsed.times;
    globalState.currentLyricsIndex = -1;
    globalState.lyricsContainerElement.classList.toggle(`${pfx}-lyrics-empty`, parsed.lines.length === 0);
    if (parsed.lines.length === 0) {
      globalState.lyricsInnerElement.innerHTML = `<div class="${pfx}-lyrics-line">暂无歌词</div>`;
      return;
    }
    globalState.lyricsInnerElement.innerHTML = parsed.lines
  ...
  function updateLyricsDisplay(currentTime) {
    const globalState = window[anzhiyuPlayerConfig.initializationFlag];
    if (!globalState || !globalState.lyricsInnerElement) return;
    const { lyricsTimes, lyricsContainerElement } = globalState;
    if (!lyricsTimes || lyricsTimes.length === 0) return;
    let activeIndex = -1;
    for (let i = lyricsTimes.length - 1; i >= 0; i--) {
      if (currentTime >= lyricsTimes[i]) {
        activeIndex = i;
        break;
      }
    }
